...
namespace LeetNES
{
    ...
    public class StolenPpu
    {
        ...
        private readonly byte[] backgroundCHRValues = new byte[256*240];
        ...
        public void drawBackground(int px, int py)
        {
            ...
            var bg_pattern_base = (ushort) ((_PPUCTRL & 0x10) == 0 ? 0x0000 : 0x1000);
            ...
            var palette_num = (byte) ((attributeTableEntry >> which_palette) & 3);
            ...
            var lowBit = (byte) ((lowBits >> (7 - tile_x)) & 1);
            var highBit = (byte) ((highBits >> (7 - tile_x)) & 1);
            var color_index = (byte) (lowBit + highBit*2);
        ...
        private void computeSpritesForScanline(int scanline)
        {
            ...
            {
                var sprite_y = (byte) (OAM[oam_index*4 + 0] + 1);
        ...
        private void drawSprites(int px, int py)
        {
            ...
            for (int spr_index = 0; spr_index < sprite_count; ++spr_index)
            {
                ...
                var attributes = (byte) (OAM[oam_index*4 + 2] & 0xE3);
                // There are several bits that are "unimplemented" (always return zero during read)
                byte sprite_x = OAM[oam_index*4 + 3];

                var palette_number = (byte) (attributes & 3);
                ...
                if (mode816)
                {
                    ...
                    int ti = px - sprite_x;
                    int tj = (py - sprite_y)%8;
                    // Tiles are only 8x8, so handle when we're looking at the second tile

                    int i = flipHorizontal ? 7 - ti : ti;
                    int j = flipVertical ? 7 - tj : tj;

                    var sprite_pattern_table_base = (ushort) ((pt_index & 1) == 0 ? 0x0000 : 0x1000);
                    ...
                    var lowBit = (byte) ((lowBits >> (7 - i)) & 1);
                    var highBit = (byte) ((highBits >> (7 - i)) & 1);
                    var color_index = (byte) (lowBit + highBit*2);
                ...
                else
                {
                    ...
                    var sprite_pattern_table_base = (ushort) ((_PPUCTRL & 8) == 0 ? 0x0000 : 0x1000);
                    byte lowBits = getPatternTable((ushort) (sprite_pattern_table_base + pt_index*16 + j));
                    byte highBits = getPatternTable((ushort) (sprite_pattern_table_base + pt_index*16 + j + 8));
        ...
        private readonly byte[] RAM = new byte[0x4000];
        ...
        private byte readRAM(ushort addr)
        {
            ...
            if (addr >= 0x2000 && addr <= 0x2FFF)
            {
                ...
                if (_cartridge.NametableMirroring == NametableMirroringMode.OneScreenLowBank)
                {
        ...
        private byte PPUDATA_ReadBuffer;
        ...
        public byte read(ushort addr)
        {
            ...
            if (addr == 0x2007)
            {
                var readAddress = (ushort) (_PPUADDR & 0x3FFF);
            ...
            Console.WriteLine("Unimplemented read to PPU @ {0:X}", addr);
            return 0;
        }
        ...
        public void write(ushort addr, byte val)
        {
            ...
            else if (addr == 0x4014)
            {
                ...
                {
                    var read_address = (ushort) ((val << 8) + ((offset + _OAMADDR) & 0xFF));
                    OAM[offset] = _memory[read_address];
                }
            }
            else
            {
